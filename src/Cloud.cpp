...
void minimum(LweSample *result, const LweSample *a, const LweSample *b,
  ...
  // tmps[0] is the result of the comparaison : 0 if a is larger, 1 if b is
  // larger select the max and copy it to the result
  for (int i = 0; i < nb_bits; i++) {
...
bool importingCiphertextsPerformingComputations(
    ...
    const std::string &answer_data_file_name) {
  try {
    // reads the cloud key from file
    FILE *cloud_key = fopen(cloud_key_file_name.c_str(), "rb");
    ...
    FILE *cloud_data = fopen(cloud_data_file_name.c_str(), "rb");
    for (int i = 0; i < 16; i++)
      import_gate_bootstrapping_ciphertext_fromFile(cloud_data, &ciphertext1[i],
                                                    params);
    for (int i = 0; i < 16; i++)
      import_gate_bootstrapping_ciphertext_fromFile(cloud_data, &ciphertext2[i],
                                                    params);
    fclose(cloud_data);
    ...
    FILE *answer_data = fopen(answer_data_file_name.c_str(), "wb");
    for (int i = 0; i < 16; i++)
      export_gate_bootstrapping_ciphertext_toFile(answer_data, &result[i],
                                                  params);
    fclose(answer_data);
    ...
    std::cerr << "[importingCiphertextsPerformingComputations] Exception: "
              << ex.what() << std::endl;
  } catch (...) {
    std::cerr
        << "[importingCiphertextsPerformingComputations] Unknown exception"
        << std::endl;
  }
  return false;
}
